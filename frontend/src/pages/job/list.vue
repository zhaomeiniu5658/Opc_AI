<template>
  <view class="job-page">
    <scroll-view class="job-scroll" scroll-y :show-scrollbar="false">
      <NavBar :title="pageMeta.title" :subtitle="pageMeta.description" />

      <view class="search-panel panel fade-in">
...
const { pageMeta } = usePageMeta('jobList')
...
const enrichedJobs = computed(() =>
  jobs.value.map((job) => ({
    ...job,
    description: job.description || jobDescriptions[job.id] || '岗位职责覆盖项目执行、跨团队协作和结果交付，详情可进入岗位页继续查看。',
...
.job-page {
  ...
  background:
    linear-gradient(150deg, rgba(94, 231, 168, 0.1) 0%, rgba(94, 231, 168, 0) 34%),
    linear-gradient(180deg, #021017 0%, #07111f 48%, #030711 100%);
}
...
.sort-tab.active {
  border-color: rgba(94, 231, 168, 0.52);
  background: rgba(94, 231, 168, 0.14);
  color: #d7fbe8;
}
...
.panel {
  ...
  border-radius: 8px;
  background:
    linear-gradient(180deg, rgba(18, 30, 60, 0.76), rgba(8, 15, 31, 0.9)), rgba(8, 15, 31, 0.82);
  box-shadow:
